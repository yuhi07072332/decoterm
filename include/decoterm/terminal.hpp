...
public:
    ...
    ~Terminal() {
        if (restore_default_style_on_exit) std::cout << reset;
    }

    auto is_style_enabled() const -> bool {
        return detail::g_style_output.is_enabled;
    }

    auto color_support() const -> ColorSupport {
